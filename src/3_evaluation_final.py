...
def main(args):
    ...
    for k in args.human_names:
        ...
        for i in range (0,label_name.__len__()):
            if i==0 and args.human_test!=True:
                label_path = args.dir_path + label_name[i] +'/'
            else:
                label_path = args.dir_path_human + label_name[i] +'/'
    ...
    print("final")
    print("total_intersection percent: ", np.mean(total_intersection))
    ...
    with open('../results/eval.csv','a', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            if args.human_test==True:
                writer.writerow(["human"])
            else:
                writer.writerow([args.model_name])
            writer.writerow([str(np.mean(total_intersection))])


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process some integers.')
    parser.add_argument("--dir-path", type=str, default=f"../results/vpds/")
    parser.add_argument("--dir-path-human", type=str, default=f"../data/vpds/")
    parser.add_argument("--replay-names",  nargs='+', default=["2351","1628","1559","6219","11251"])
    parser.add_argument("--model-name", type=str, default="rcnn_ROCI")
    parser.add_argument("--human-names", nargs='+', default=["1","2","3","4","5"])
